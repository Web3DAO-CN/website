...
import { ChevronDown, ExternalLinkAlt } from 'components/FontawesomeIcon'
...
const FlyoutRowActiveIndicator = styled.div`
  ...
  width: 9px;
`
// const LinkOutCircle = styled(ArrowAltCircleDown)`
...
function Row({
  ...
  const chainInfo = CHAIN_INFO?.[targetChain]
  if (!chainInfo) {
    return <></>
  }
  const { helpCenterUrl, explorer, bridge, label, logoUrl, faucet } = chainInfo
        ...
        <ActiveRowLinkList>
          {bridge ? (
            <ExternalLink href={bridge}>
              <BridgeLabel chainId={chainId} /> <ExternalLinkAlt />
            </ExternalLink>
          ) : null}
          {explorer ? (
            <ExternalLink href={explorer}>
              <ExplorerLabel chainId={chainId} /> <ExternalLinkAlt />
            </ExternalLink>
          ) : null}
          {helpCenterUrl ? (
            <ExternalLink href={helpCenterUrl}>
              <Trans>Help Center</Trans> <ExternalLinkAlt />
            </ExternalLink>
          ) : null}
          {faucet ? (
            <ExternalLink href={faucet} className="items-center flex justify-between text-[#415BE5]">
              <Trans>Faucet</Trans> <ExternalLinkAlt />
            </ExternalLink>
          ) : null}
        </ActiveRowLinkList>
      </ActiveRowWrapper>
